...
{
    pthread_once(&once_control, MimeType::init);//使用pthread_once实现线程安全单例
...
HttpData::HttpData(EventLoop *loop, int connfd):
        loop_(loop),//设置连接关联的EventLoop
        channel_(new Channel(loop, connfd)),//创建连接对应的channel
        fd_(connfd),//设置连接关联的套接字
        error_(false),//设置错误标志为false
        connectionState_(H_CONNECTED),//设置被连接状态
        method_(METHOD_GET),//初始化请求类型
        HTTPVersion_(HTTP_11),//初始化协议版本
        nowReadPos_(0), //设置缓冲区对应的读标记
        state_(STATE_PARSE_URI), 
        hState_(H_START),
        keepAlive_(false)//是否维持keepalive
{
    /**
     * 注册当前连接对应的事件处理函数
     */
...
{
    connectionState_ = H_DISCONNECTED;//设置连接状态为断开状态
...
{
    channel_->setEvents(DEFAULT_EVENT);//设置channel关注的默认事件
    loop_->addToPoller(channel_, DEFAULT_EXPIRED_TIME);//把连接channel添加到epoll上
}
