...
std::vector<pthread_t> ThreadPool::threads;//线程队列
std::vector<ThreadPoolTask> ThreadPool::queue;//任务队列
int ThreadPool::thread_count = 0;//当前线程数量
int ThreadPool::queue_size = 0;//任务队列的大小
int ThreadPool::head = 0;
int ThreadPool::tail = 0;
int ThreadPool::count = 0;
int ThreadPool::shutdown = 0;//是否已经关闭标志
int ThreadPool::started = 0;//被启动的线程个数
...
int ThreadPool::threadpool_create(int _thread_count, int _queue_size)
{
    ...
    do
    {
        ...
        threads.resize(_thread_count);// 初始化线程的个数
        queue.resize(_queue_size);//初始化任务队列的个数
