...
{
    struct timeval now;
    gettimeofday(&now, NULL);//获取当前精确时间
    expiredTime_ = (((now.tv_sec % 10000) * 1000) + (now.tv_usec / 1000)) + timeout;
}
...
void TimerManager::handleExpiredEvent()
{
    ...
    while (!timerNodeQueue.empty())
    {
        ...
        if (ptimer_now->isDeleted())//如果该时间节点已经被设置了删除标志
            timerNodeQueue.pop();
        else if (ptimer_now->isValid() == false)//如果该时间节点已经超时
            timerNodeQueue.pop();
        else
            break;
    }
